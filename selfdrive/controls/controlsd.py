...
class Controls:
  ...
  def update_events(self, CS):
    ...
    if not self.nn_alert_shown and self.sm.frame % 600 == 0 and self.CP.lateralTuning.which() == 'torque' and self.CI.has_lateral_torque_nn:
  ...
  def state_control(self, CS):
    ...
    if self.always_on_lateral:
      ...
      CC.alwaysOnLateral = self.lateral_allowed and gear_check
      if CC.alwaysOnLateral:
        self.current_alert_types.append(ET.WARNING)

    # Check which actuators can be enabled
    signal_check = not (self.pause_lateral_on_signal and self.sm['lateralPlan'].laneChangeState != LaneChangeState.off)
